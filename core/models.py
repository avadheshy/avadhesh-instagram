...
class Post(models.Model):
    text = models.CharField(max_length=140, blank=True,
                            null=True,)  # null == None, blank = ''
    # BASE_DIR -> media -> post_images
    img = models.ImageField(upload_to='post_images/')
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='user_user')    # user_id
    created_on = models.DateTimeField(auto_now_add=True)
    updated_on = models.DateTimeField(auto_now=True)
    liked = models.ManyToManyField(
        User, default=0, blank=True, related_name='user_liked')
